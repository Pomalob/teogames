...
def calculate_emv_and_risk(tree):
    ...
    for strategy, environments in tree.items():
        ...
        variance = sum(p * (v - emv) ** 2 for p, v in leaves)
...
if __name__ == "__main__":
    ...
    for s, d in results.items():
        ratio = d['emv'] / d['sigma'] if d['sigma'] > 0 else float('nan')
        print(f"{s:<22} {d['emv']:>14,.0f} {d['sigma']:>14,.0f} {ratio:>10.3f}")

    # Лучшая стратегия по соотношению EMV/sigma (игнорируем нулевую sigma)
    best = max(
        (s for s in results if results[s]['sigma'] > 0),
        key=lambda s: results[s]['emv'] / results[s]['sigma'],
    )
    ...
    plt.close()
    print("\n[График сохранён: task3_decision_tree.png]")
