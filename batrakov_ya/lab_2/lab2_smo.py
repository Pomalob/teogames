...
Студент: Батраков Я. А.

Задача 1. Многоканальная СМО с отказами (M/M/k/0)
  α=1.5 ч, n=48 зявок/сутки, k=5 каналов

Задача 2. Многоканальная СМО с неограниченной очередью (M/M/k/∞)
  λ=250 зявок/сутки, t=12 мин, α_c=3 (коэф. затрат), n=3 (для P(очередь≤n))

Задача 3. Многоканальная СМО с ограниченной очередью (M/M/k/k+n)
  λ=7 зявок/ч, t=2 мин, k=8 каналов, n=5 мест в очереди, T=10 ч, C=70 у.е.

Задача 4. СМО с «нетерпеливыми» заявками (ω-предел)
  λ=1.2 зявок/мин, t=1.5 мин, k=2 канала, ω=5 мин, C=180 у.е., ε=0.001

Задача 5. Замкнутая одноканальная СМО (система Энгсета, M/M/1//N)
  N=16 источников, k=2 заявки/мес на источник, t=1.0 день, P_min=85%

Задача 6. Замкнутая многоканальная СМО (M/M/k//N)
  k=4 канала, N=18 источников, λ=2.1 зявок/ч от источника, t=0.4 ч
"""
...
def task1():
    ...
    alpha = 1.5      # среднее время обслуживания, ч
    n_day = 48       # заявок в сутки
    k_given = 5      # заданное число каналов
    ...
    plt.tight_layout()
    plt.savefig(SAVE_DIR / 'task1_plot.png', dpi=120)
    plt.close()
    print("\n    [График сохранён: task1_plot.png]")

    print(f"\nВыводы: Система с отказами при λ={lam:.3f} зявок/ч и μ={mu:.3f} зявок/ч."
...
def task2():
    ...
    lam_day = 250       # заявок/сутки
    t_min = 12          # среднее время обслуживания, мин
    alpha_c = 3         # коэффициент затрат
    n_queue = 3         # для расчёта P(очередь ≤ n)
    ...
    print(f"\n2b. Поиск k_opt (мин C(k)=k/α_c+W):")
    ...
    ax.legend(); ax.grid(True)
    plt.tight_layout()
    plt.savefig(SAVE_DIR / 'task2_plot.png', dpi=120)
    plt.close()
...
def task3():
    ...
    n = 5           # мест в очереди
    T_work = 10     # ч работы
    C_cost = 70.0   # у.е. за одну обслуженную заявку
    ...
    L = L_q + L_obsl
    W_q = (L_q / lam_abs) * 60 if lam_abs > 0 else 0
    ...
    print(f"\nВыводы: При k={k} каналах и λ={lam} зявок/ч нагрузка ρ={lam/(k*mu):.4f}."
          f"\n  Отказы: P_отк={p_otk:.4f}, потери={S_loss:.1f} у.е. за смену.")
...
def task4():
    ...
    t = 1.5         # мин
    k = 2           # каналов
    omega = 5.0     # макс. время ожидания, мин
    C_income = 180.0
    eps = 0.001
    ...
    for j_i, p in enumerate(probs[:min(J, 20)]):
    ...
    nu_leave = L_q / omega
    lam_served = lam - nu_leave

    P_served = lam_served / lam
    W_q = L_q / lam   # Little's law: L_q = λ·W_q
    W_obsl = t
    W = W_q + W_obsl
...
def task5():
    ...
    N = 16         # источников заявок
    k_month = 2    # заявок в месяц от одного источника
    t_days = 1.0   # дней на обслуживание
    P_min = 85.0   # минимальный % активных источников
    ...
    threshold = math.ceil(P_min / 100 * N)
    max_broken = N - threshold
...
def task6():
    ...
    k = 4           # каналов
    N = 18          # источников
    lam_src = 2.1   # зявок/ч от одного активного источника
    t = 0.4         # ч (время обслуживания)
    ...
    A = mu * L_obsl
    Q = A / (N * lam_src) if N > 0 else 0

    W_q = (L_q / A) * 60 if A > 0 else 0
    W_obsl = t * 60
    W = (L / A) * 60 if A > 0 else 0
    ...
    ax.grid(True, axis='y')
    plt.tight_layout()
    plt.savefig(SAVE_DIR / 'task6_plot.png', dpi=120)
    plt.close()
